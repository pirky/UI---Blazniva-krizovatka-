...
def print_crossroad_object(crossroad):
...
def max_step_object(car, crossroad, direction):
    steps = 0
    if direction == "right":
...
def move_combo_object(state, iteror, visited, stack, depth, direction):
    steps = max_step_object(state.cars[iteror], state.crossroad, direction)
    temp = deepcopy(state)
    temp.depth += 1
    if temp.depth > depth:
        return 0
...
def dfs_object(first_state, depth):
    stack = [first_state]
    visited = []
    while True:
        if len(stack) == 0:
        ...
        for i in range(len(state.cars)):
            if state.cars[i].direction == "h":
                end = move_combo_object(state, i, visited, stack, depth, "right")
                if end == 1:
                    print_crossroad_object(stack.pop().crossroad)
                    return 1
                move_combo_object(state, i, visited, stack, depth, "left")
            elif state.cars[i].direction == "v":
                move_combo_object(state, i, visited, stack, depth, "down")
                move_combo_object(state, i, visited, stack, depth, "up")

    return 0


def iterative_dfs_object():
    for depth in range(18):
        state = init_object()
        end = dfs_object(state, depth)
        if end == 1:
            return
